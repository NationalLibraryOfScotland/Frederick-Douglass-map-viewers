   ...
   var edin1849 = new ol.layer.Tile({
	...
	// the golden circle selected style 
	   ...
	   // this is the layer definition for the main points GEOJSON file showing the locations of Frederick Douglass in Edinburgh

	    var circles = new ol.layer.Vector({
        	  preload: Infinity,
        	  visible: true,
		  source: new ol.source.Vector({
		   url: 'https://geo.nls.uk/maps/douglass/scripts/frederick_douglas_edinburgh.js',
	...
	// the main ol map class
		...
		var map = new ol.Map({
		  ...
		  layers: [OS1900sGBback, os25scotland1, edin1849, circles],
		  logo: false,
                  loadTilesWhileAnimating: true,
		  view: new ol.View({
		    center: ol.proj.transform([-3.196, 55.952], 'EPSG:4326', 'EPSG:3857'),
		    zoom: 14,
		    extent: maxExtent,
		    minZoom: 13,
		    maxZoom: 20
...
// function executed on map click to select features and highlight them in the leftpanel
	...
	map.on('click', function(event) {
		...
		if (selectedFeatures.length > 0 ) { 	unselectPreviousFeatures(); }
